...
- [ ] **DCFG-01**: System supports pluggable domain configurations via YAML schema defining entity types, relation types, and extraction prompts per domain
- [ ] **DCFG-02**: Contract domain ontology defines entity types (Party, Obligation, Deadline, Cost, Clause, Service, Venue) and relation types (OBLIGATES, CONFLICTS_WITH, DEPENDS_ON, COSTS, PROVIDES, RESTRICTS)
- [ ] **DCFG-03**: Domain-specific extraction prompt templates guide Claude to extract contract-relevant entities and relations
- [ ] **DCFG-04**: Existing biomedical extraction pipeline (Scenarios 1-6) continues to work unchanged — biomedical migration to domain config deferred to v2 after contract domain proves the pattern
