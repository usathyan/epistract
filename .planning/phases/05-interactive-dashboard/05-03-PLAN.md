...
Output: Working /api/chat endpoint with SSE streaming, system prompt assembly, multi-turn context management.
</objective>
...
<task type="auto">
  <name>Task 2: Create chat SSE endpoint and wire into server</name>
  <files>scripts/workbench/api_chat.py, scripts/workbench/server.py</files>
  <read_first>
    - scripts/workbench/server.py
    - scripts/workbench/system_prompt.py
    - scripts/workbench/data_loader.py
...
"</automated>
  ...
  <done>Chat endpoint streams Claude responses via SSE with full KG context and multi-turn history; wired into FastAPI server</done>
</task>
