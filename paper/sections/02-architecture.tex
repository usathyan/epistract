...
Critically, validation is not merely a filter --- it is also an enrichment step. Each validated molecular identifier becomes a new graph node (\texttt{CHEMICAL\_STRUCTURE}, \texttt{PEPTIDE\_SEQUENCE}, or \texttt{NUCLEOTIDE\_SEQUENCE}) linked to its parent entity via a \texttt{HAS\_STRUCTURE} or \texttt{HAS\_SEQUENCE} relation. These structural nodes carry computed properties as attributes --- molecular formula, hydrogen bond donors and acceptors, isoelectric point, GC content --- making them queryable and filterable. This means the knowledge graph contains verified molecular structures as first-class entities, not text annotations.
...
\begin{enumerate}
  \item \textbf{Core Pipeline} (\texttt{core/}): Domain-agnostic document ingestion, entity extraction orchestration, graph construction (via sift-kg), and epistemic analysis dispatch. This layer contains no domain-specific logic.
  \item \textbf{Domain Configurations} (\texttt{domains/}): Pluggable packages containing entity/relation schemas (YAML), extraction prompts (SKILL.md), and domain-specific epistemic rules (Python). Each domain package defines what to extract and how to analyze it.
  \item \textbf{Example Consumers} (\texttt{examples/}): Application frontends (web dashboard, Telegram bot) that work with any domain's graph output, demonstrating how extracted knowledge can be presented and queried.
\end{enumerate}

Adding a new domain requires creating a configuration package --- no modifications to the core pipeline. A domain creation wizard (\texttt{/epistract:domain}) automates this process by analyzing sample documents and generating a complete domain package including the entity/relation schema, extraction prompt, and epistemic analysis rules for both the brute facts and epistemic super-domain layers.

This separation enables two distinct user paths: domain \emph{users} who install a pre-built domain and run the pipeline immediately, and domain \emph{developers} who create new domain packages for their specific document corpora.
