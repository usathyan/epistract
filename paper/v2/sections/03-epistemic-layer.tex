\section{The Epistemic Super-Domain Layer}
\label{sec:epistemic}

The epistemic super-domain layer is the central architectural contribution of v3. This section describes its rationale, taxonomy, rule engine, output artifact, and the persona-driven narrator stage that consumes it.
...
\paragraph{Four-level FDA epistemology (fda-product-labels).} FDA Structured Product Labels carry a recognizable epistemic vocabulary (``has been demonstrated,'' ``has been reported,'' ``has been theorized,'' implicit assertions in the Indications and Usage section). The fda-product-labels epistemic module implements a four-level classifier mapping onto the shared taxonomy with finer granularity: \texttt{established} (regulatory-grade asserted), \texttt{reported} (lower-tier asserted), \texttt{theoretical} (hypothesized in the FDA-specific sense), and a default \texttt{asserted} for unmarked indications-and-usage statements. The four levels are visible to the analyst persona, which can stratify briefings accordingly.
