...
The framework has been validated across eight scenarios spanning three of the four pre-built domains. This section presents per-scenario quantitative results, schema coverage analysis, epistemic-layer outputs, and an honest accounting of what the evaluation does and does not establish.

\subsection{Scenarios across domains}

Table~\ref{tab:scenarios} reports the eight validation scenarios. The first six (S1--S6) exercise the drug-discovery domain across varied therapeutic areas; S7 is the inaugural clinicaltrials scenario; S8 is the inaugural fda-product-labels scenario.

\begin{table}[t]
\centering
\small
\caption{Validation scenarios across active domains, reporting the most recent rebuild of each. ``UATs'' is the per-scenario validation pass rate (user-acceptance tests for S1--S7; Pydantic-strict extraction pass rate for S8 since the fda-product-labels scenario uses extraction-write-time validation rather than UAT-style functional regression). ``Rebuild'' indicates which framework state produced the reported numbers; the version-progression table for the most heavily rebuilt scenario (S6) appears as Table~\ref{tab:s6-enrichment} below. S1--S5 numbers are V2 rebuilds documented in \texttt{tests/scenarios/scenario-0\{1--5\}-*-v2.md}; S6 is the V3 rebuild (Table~\ref{tab:s6-enrichment}); S7 is the V3.1 launch; S8 is the V3.2.1 launch.}
\label{tab:scenarios}
\begin{tabular}{@{}clrrrrcc@{}}
\toprule
\textbf{\#} & \textbf{Domain / Corpus} & \textbf{Docs} & \textbf{Nodes} & \textbf{Relations} & \textbf{Comm.} & \textbf{UATs} & \textbf{Rebuild} \\
\midrule
S1 & drug-discovery / Neurogenetics (PICALM)        & 15 & 183 & 478 & 6  & 4/4 & V2 \\
S2 & drug-discovery / Oncology (KRAS G12C)          & 16 & 140 & 432 & 4  & 5/5 & V2 \\
S3 & drug-discovery / Rare Disease                  & 15 & 110 & 278 & 4  & 3/3 & V2 \\
S4 & drug-discovery / Immuno-Oncology               & 16 & 151 & 440 & 5  & 4/4 & V2 \\
S5 & drug-discovery / CV \& Inflammation            & 15 &  90 & 245 & 4  & 3/3 & V2 \\
S6 & drug-discovery / GLP-1 CI                      & 34 & 278 & 855 & 10 & 6/6 & V3 \\
S7 & clinicaltrials / GLP-1 Phase 3 (CT.gov)         & 10 & 142 & 395 & 8  & 5/5 & V3.1 \\
S8 & fda-product-labels / openFDA SPL                & 7  &  81 & 149 & 3  & 7/7 & V3.2.1 \\
\midrule
   & \textbf{Drug-discovery (S1--S6)}               & 111 & 952 & 2{,}728 & 33 & 25/25 &        \\
   & \textbf{Clinicaltrials (S7)}                   & 10  & 142 & 395     & 8  & 5/5   &        \\
   & \textbf{Fda-product-labels (S8)}               & 7   &  81 & 149     & 3  & 7/7   &        \\
\bottomrule
\end{tabular}
\end{table}

The drug-discovery domain has accumulated the most pressure-testing. S1--S5 establish breadth across therapeutic areas; S6 added multi-source corpus assembly (PubMed E-utilities, Google Scholar, Google Patents via SerpAPI) and stress-tested patent-document handling. S7 is the first scenario in a second domain and exercises clinicaltrials' phase-based evidence grading and CT.gov ingestion. S8 is the first scenario in a third domain and exercises fda-product-labels' four-level FDA epistemology classifier on real openFDA SPL labels. Contracts is the only scaffolded domain that does not yet have a validated scenario.
...
\subsection{FDA Structured Product Labels (S8)}

The S8 corpus comprises 7 openFDA SPL records hand-selected to exercise the full range of FDA label-section vocabulary: Ozempic (NDA209637 injectable, GLP-1 RA cardiovascular outcomes), Wegovy (NDA215256 weight management), Mounjaro (NDA215866 dual GIP/GLP-1 agonist), Humira (BLA125057 with explicit BOXED WARNING for serious infections / malignancy), Gleevec (NDA021588 with survival-benefit clinical-studies data), Lipitor (NDA020702 with LDL-lowering efficacy), and Jantoven (ANDA040416 with INR-monitoring and drug-interaction breadth). The fda-product-labels domain extracted 81 nodes and 149 relations across 14 entity types, with the largest contributions from \texttt{ADVERSE\_REACTION} (21), \texttt{INACTIVE\_INGREDIENT} (15), and \texttt{DRUG\_INTERACTION} (13).

The four-level FDA epistemology classifier produced a tier distribution that itself is a finding about SPL labels: \emph{0 \texttt{established}, 4 \texttt{observed}, 145 \texttt{reported}, 0 \texttt{theoretical}}. The 97\% dominance of \texttt{reported} relations reflects the structural reality that post-marketing pharmacovigilance fills the bulk of the ADVERSE REACTIONS, DRUG INTERACTIONS, and POSTMARKETING sections. The 4 \texttt{observed} relations capture RCT-cited efficacy from CLINICAL STUDIES sections (SUSTAIN-6 in Ozempic, LDL-lowering in Lipitor, survival benefit in Gleevec, INR monitoring in Jantoven). The absence of \texttt{established} relations as a tier-attributed category does not mean the corpus lacks boxed warnings: the graph captures them as \texttt{WARNING} and \texttt{CONTRAINDICATION} entity nodes rather than as a tier flag on a relation edge. The absence of \texttt{theoretical} relations reflects the SPL sections processed -- mechanism-of-action prominence is light in the seven labels selected. This four-tier distribution is in turn the subject of the 1{,}571-word narrator briefing's executive summary.

S8 was contributed via PR \#18 by Chris Davidson, ported via the controlled-port pattern documented in Section~\ref{sec:collaboration}; it closes GitHub Issue~\#14, which the v3.2.0 framework had tracked as the inaugural showcase corpus for fda-product-labels.

\subsection{Schema coverage analysis}

The drug-discovery schema has gone through one consolidation. The S1--S5 V2 rebuilds ran against an earlier 17-entity / 30-relation form and together exercised 17 of 17 entity types ($100\%$) and 28 of 30 relation types ($93\%$); the two unused relations were \texttt{PHOSPHORYLATES} and \texttt{METABOLIZED\_BY}, which represent specialized biochemical relationships rarely discussed at the abstract level. The schema was subsequently consolidated to 13 entities / 22 relations by removing low-yield types observed across S1--S5; the consolidated schema retained the high-yield types and merged or removed the low-yield ones. The S6 V3 rebuild on the consolidated schema exercised all 13 entity types and 20 of 22 relation types (the two unused being domain-rare structural relations). On the clinicaltrials domain, S7 exercised 11 of 12 entity types ($92\%$, the unused being \texttt{Endpoint} as a separate node from \texttt{Outcome}) and 9 of 10 relation types ($90\%$). On the fda-product-labels domain, S8 exercised 14 of 17 entity types ($82\%$, the unused being SPL-section types not present in the 7 labels selected: \texttt{NDC}, \texttt{LotNumber}, and \texttt{StorageInstruction}), with full coverage of the four-level epistemology classifier on all 149 extracted relations.
...
\item \emph{Universal coverage of contracts.} The contracts domain is the only one of the four pre-built domains without a validated showcase corpus. Its schema is designed but not pressure-tested. (Fda-product-labels was scaffolded in v3.2.0 and validated via S8 in v3.2.1, closing the prior Issue~\#14 gap.)
